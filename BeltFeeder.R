...
#Importar Datos de Excel de Belt Feeder
datos<- read_excel(ruta_excel,sheet = "BeltFeeder")
...
#a_exp<- fitdistcens(f_w, "exp") # No se ajusto a los parametros de una exponencial
...
cdfcompcens(list(a_W,a_n,a_ln), legendtext = plot.legend,main = "Tiempo entre fallas Electricas")
...
f_d <- data.frame(
  left = tiempo_M, 
  right = ifelse(status_M == 1, tiempo_M, NA)
)
...
cdfcompcens(list(a_W2,a_n2,a_ln2,a_exp2), legendtext = plot.legend,main = "Tiempo entre fallas Mecanicas")


# Graficamos Distribuciones obtenidas
tiempos_1 <- seq(min(tiempo_M), max(tiempo_M), length.out = 1000)
...
par(mfrow = c(1,1), mar = c(4,4,2,1))
hist(tiempo_M, prob = TRUE, col = "lightgray",
...
f_i <- data.frame(
  left = tiempo_I, 
  right = ifelse(status_I == 1, tiempo_I, NA)
)
...
# Distribuciones tiempo entre fallas INSTRUMENTACION
a_W3 <- fitdistcens(f_I, "weibull")
a_n3 <- fitdistcens(f_I, "norm")
a_ln3 <- fitdistcens(f_I, "lnorm")
#a_exp3<- fitdistcens(f_I, "exp") # No ajusta a una exponencial
...
par(mfrow = c(1,1), mar = c(4,4,2,1))
plot.legend <- c("Weibull","normal", "lognormal")
cdfcompcens(list(a_W3,a_n3,a_ln3), legendtext = plot.legend,main = "Tiempo entre fallas Instrumentacion")
...
par(mfrow = c(1,1), mar = c(4,4,2,1))
hist(tiempo_I, prob = TRUE, col = "lightgray",
     main = "Comparación de distribuciones",
     xlab = "Horas", ylab = "Densidad")
lines(tiempos_2, den_weibull2, col = "red", lwd = 2)
lines(tiempos_2, den_normal2, col = "blue", lwd = 2)
lines(tiempos_2, den_log2, col = "green", lwd = 2)
legend("topright",
       legend = c("Weibull", "Normal", "Lognormal"),
       col = c("red", "blue", "green"),
       lwd = 2)
...
#a_exp4<- fitdistcens(f_O, "exp")  #No toma la exponencial
...
hist(falla_E, breaks = 10,  prob = TRUE, main ="Histograma y curva aproximada", ylab = "Densidad")
lines(tiemposF_1,den_gamma4,col = "red",lwd = 2)
par(mfrow = c(1,1), mar = c(4,4,2,1))
hist(falla_E, prob = TRUE, col = "lightgray", ylim = c(0,max_y10),
     main = "Comparación de distribuciones",
     xlab = "Horas", ylab = "Densidad")
...
tln1 <- fitdist(falla_M,"lnorm")
tg1 <- fitdist(falla_M,"gamma")
texp1 <- fitdist(falla_M,"exp")
...
par(mfrow = c(1,1), mar = c(4,4,2,1))
plot.legend <- c("lognormal","gamma","exponecial")
cdfcomp(list(tln1,tg1,texp1),legendtext = plot.legend,main = "Tiempo reparacion fallas mecanicas")


# Graficamos Distribuciones obtenidas
tiemposF_2 <- seq(min(falla_M), max(falla_M), length.out = 1000)
...
# Histograma
par(mfrow = c(1,1), mar = c(4,4,2,1))
max_y11 <- max(hist(falla_M, plot=FALSE)$density, den_log5)
hist(falla_M, breaks = 15, prob = TRUE, main ="Histograma y curva aproximada", ylab = "Densidad")
lines(tiemposF_2,den_log5,col = "red",lwd = 2)
par(mfrow = c(1,1), mar = c(4,4,2,1))
hist(falla_M, prob = TRUE, col = "lightgray", ylim = c(0,max_y11),
     main = "Comparación de distribuciones",
     xlab = "Horas", ylab = "Densidad")
lines(tiemposF_2, den_log5, col = "red", lwd = 2)
...
tln2 <- fitdist(falla_I,"lnorm")
tg2 <- fitdist(falla_I,"gamma")
texp2 <- fitdist(falla_I,"exp")
...
par(mfrow = c(1,1), mar = c(4,4,2,1))
plot.legend <- c("lognormal","gamma","exponecial")
cdfcomp(list(tln2,tg2,texp2),legendtext = plot.legend, main = "Tiempo reparacion fallas instrumentacion")

# Graficamos Distribuciones obtenidas
tiemposF_3 <- seq(min(falla_I), max(falla_I), length.out = 1000)
den_log6 <- dlnorm(tiemposF_3, meanlog = tln2$estimate["meanlog"],sdlog = tln2$estimate["sdlog"])
den_gamma6 <-dgamma(tiemposF_3,shape = tg2$estimate["shape"], rate = tg2$estimate["rate"])
den_exp6 <- dexp(tiemposF_3, rate = texp2$estimate["rate"])
par(mfrow = c(2,2), mar = c(4,4,2,1))
plot(tiemposF_3,den_log6,main = "Lognormal", xlab = "Horas")
plot(tiemposF_3,den_gamma6,main = "gamma", xlab = "Horas")
plot(tiemposF_3,den_exp6,main = "Exponencial", xlab = "Horas")
...
lines(tiemposF_3, den_exp6, col = "green", lwd = 2)
legend("topright",
       legend = c("Lognormal", "gamma", "Exponencial"),
       col = c("red", "blue", "green"),
       lwd = 2)
...
# Histograma
par(mfrow = c(1,1), mar = c(4,4,2,1))
max_y12 <- max(hist(falla_O, plot=FALSE)$density, den_log7)
hist(falla_O, prob = TRUE, main ="Histograma y curva aproximada", ylab = "Densidad")
lines(tiemposF_4,den_exp7,col = "red",lwd = 2)
par(mfrow = c(1,1), mar = c(4,4,2,1))
hist(falla_O, prob = TRUE, col = "lightgray", ylim = c(0,max_y12),
     main = "Comparación de distribuciones",
     xlab = "Horas", ylab = "Densidad")
...
#a_exp5<- fitdistcens(f_MA, "exp") #No ajusta a una exponencial
...
par(mfrow = c(1,1), mar = c(4,4,2,1))
plot.legend <- c("Weibull","normal", "lognormal")
cdfcompcens(list(a_W5,a_n5,a_ln5), legendtext = plot.legend,main = "Tiempo entre Mantenciones")
